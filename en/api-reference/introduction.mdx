---
title: "Tylon public API"
sidebarTitle: "Introduction"
description: "Base URL, what the API reads and writes, how a board is chosen, and the rate limit. The reference for anything reaching Tylon without a browser."
---
...
**It reads and it writes.** Reads need nothing beyond the credential. Writes need the `write` scope and an `Idempotency-Key` — moving a card runs Git, and a retry that cannot tell "it failed" from "the answer got lost" must not be able to merge twice. See [Writing](/en/api-reference/writing).
...
<ResponseField name="Authorization" type="header" required>
  `Bearer` followed by the secret, which starts with `tyl_sk_`. See [Authentication](/en/api-reference/authentication).
</ResponseField>

<ResponseField name="X-Tylon-Workspace" type="header">
  The id of the board the call is about. Optional when the credential reaches one. See [Choosing a board](/en/api-reference/workspaces).
</ResponseField>
...
120 requests per minute, **per credential**. Over it, `429`.

Counted by the credential and not by the address it comes from, which is what
makes the number mean something: two integrations on one CI runner do not
throttle each other, and one integration spread over a hundred machines does
not get a hundred budgets.

There is no per-plan tier. If you need more for something real, say so rather
than issuing extra credentials to get around it — the limit is a guard against
runaways, not a lever.
